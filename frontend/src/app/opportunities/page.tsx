...
const fieldClass =
  "w-full rounded-md border border-slate-300 bg-white px-3 py-2 text-sm text-slate-900 outline-none transition focus:border-slate-500 focus:ring-2 focus:ring-slate-200 dark:border-slate-700 dark:bg-slate-950 dark:text-slate-100 dark:focus:border-slate-500 dark:focus:ring-slate-800";
...
function EvidenceItem({ item, index }: { item: Record<string, unknown>; index: number }) {
  ...
  return (
    <li className="min-w-0 border-l-2 border-slate-200 pl-3 dark:border-slate-700">
      <div className="flex flex-wrap items-center gap-x-2 gap-y-1">
        <p className="min-w-0 break-words text-sm font-medium text-slate-900 dark:text-slate-100">{title}</p>
        {url ? (
          <a
            href={url}
            target="_blank"
      ...
      {detail ? <p className="mt-1 break-words text-sm text-slate-600 dark:text-slate-300">{displayValue(detail)}</p> : null}
      {timestamp ? <p className="mt-1 text-xs text-slate-400">{formatDate(String(timestamp))}</p> : null}
    </li>
  );
}
      ...
      <main className="mx-auto min-w-0 w-full max-w-[1440px] px-4 py-7 sm:px-6 lg:px-8">
        <header className="flex flex-col gap-4 border-b border-slate-200 pb-5 dark:border-slate-800 lg:flex-row lg:items-end lg:justify-between">
          ...
          <div className="flex flex-wrap items-center gap-2">
            <button
              ...
              className="inline-flex h-9 items-center gap-2 rounded-md border border-slate-300 bg-white px-3 text-sm font-medium hover:bg-slate-50 dark:border-slate-700 dark:bg-slate-950 dark:hover:bg-slate-900"
            >
            ...
            <button
              ...
              disabled={busy === "run"}
              className="inline-flex h-9 items-center gap-2 rounded-md bg-slate-900 px-3 text-sm font-medium text-white hover:bg-slate-700 disabled:opacity-50 dark:bg-slate-100 dark:text-slate-950 dark:hover:bg-white"
            >
        ...
        <section className="grid grid-cols-2 gap-px border-b border-slate-200 bg-slate-200 sm:grid-cols-4 dark:border-slate-800 dark:bg-slate-800">
          {[
            ...
            <div key={label} className="bg-panel-muted px-4 py-4">
        ...
        <div className="flex gap-1 border-b border-slate-200 py-4 dark:border-slate-800" role="tablist" aria-label="Opportunity workspace views">
          {([
            ["queue", "Queue"],
            ["universe", "Universe"],
            ["runs", "Run history"],
            ...
            <button
              ...
              className={`rounded-md px-3 py-2 text-sm font-medium ${tab === value ? "bg-slate-900 text-white dark:bg-slate-100 dark:text-slate-950" : "text-slate-500 hover:bg-slate-100 hover:text-slate-900 dark:hover:bg-slate-900 dark:hover:text-slate-100"}`}
            >
              {label}
            </button>
          ))}
        ...
        {!loading && tab === "queue" ? (
          <div className="grid min-h-[620px] min-w-0 grid-cols-1 gap-6 py-6 xl:grid-cols-[380px_minmax(0,1fr)]">
            <aside className="min-w-0 border-r-0 border-slate-200 xl:border-r xl:pr-6 dark:border-slate-800">
                ...
                {candidateFilters.map((item) => {
                  ...
                  return (
                    <button
                      ...
                      className={`shrink-0 rounded-md border px-2.5 py-1.5 text-xs font-medium ${filter === item.value ? "border-slate-900 bg-slate-900 text-white dark:border-slate-100 dark:bg-slate-100 dark:text-slate-950" : "border-slate-200 bg-white text-slate-600 dark:border-slate-800 dark:bg-slate-950 dark:text-slate-300"}`}
                    >
                      {item.label} {count}
                    </button>
                  );
              ...
              <div className="space-y-2">
                {filteredCandidates.length === 0 ? (
                  <div className="min-w-0 border border-dashed border-slate-300 px-4 py-8 text-center dark:border-slate-700">
                    <CircleDot className="mx-auto h-5 w-5 text-slate-400" />
                    <p className="mt-2 text-sm font-medium">No candidates in this state</p>
                    <p className="mt-1 break-words text-xs text-slate-500">Run discovery after adding securities to the universe.</p>
                  </div>
                ) : (
                  filteredCandidates.map((candidate) => (
                    <button
                      ...
                      className={`w-full border p-4 text-left transition ${selectedId === candidate.id ? "border-slate-900 bg-white dark:border-slate-100 dark:bg-slate-950" : "border-slate-200 bg-transparent hover:border-slate-400 dark:border-slate-800 dark:hover:border-slate-600"}`}
                    >
                      ...
                      <div className="mt-3 flex flex-wrap items-center gap-2">
                        ...
                        {candidate.signal_stage ? <span className="text-xs text-slate-400">{displayValue(candidate.signal_stage)}</span> : null}
                      </div>
                      <p className="mt-3 line-clamp-2 break-words text-xs leading-5 text-slate-500 dark:text-slate-400">{candidate.why_now}</p>
                    </button>
                  ))
                )}
              </div>
            ...
            <section className={`min-w-0 ${filteredCandidates.length === 0 ? "hidden xl:block" : ""}`}>
              {!selected ? (
                <div className="flex min-h-64 items-center justify-center border border-dashed border-slate-300 text-sm text-slate-500 dark:border-slate-700">Select a candidate to inspect its evidence and thesis.</div>
              ) : (
                <div className="min-w-0 space-y-6">
                    ...
                    <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                      <div className="min-w-0">
                        <div className="flex flex-wrap items-center gap-2">
                          ...
                          {selected.signal_stage ? <span className="text-xs text-slate-500">Stage: {displayValue(selected.signal_stage)}</span> : null}
                        </div>
                        <h2 className="mt-2 break-words text-xl font-semibold sm:text-2xl">{selected.title}</h2>
                        <p className="mt-2 text-xs text-slate-400">Captured {formatDate(selected.captured_at)} · expires {formatDate(selected.expires_at)}</p>
                      </div>
                      <div className="shrink-0 text-left sm:text-right">
                        <p className="text-xs font-semibold uppercase text-slate-400">Priority score</p>
                  ...
                  <div className="grid gap-6 lg:grid-cols-2">
                    <section className="min-w-0">
                      <h3 className="text-xs font-semibold uppercase text-slate-400">Why now</h3>
                      <p className="mt-2 break-words text-sm leading-6 text-slate-700 dark:text-slate-200">{selected.why_now}</p>
                    </section>
                    <section className="min-w-0">
                      <h3 className="text-xs font-semibold uppercase text-slate-400">Investable thesis</h3>
                      <p className="mt-2 break-words text-sm leading-6 text-slate-700 dark:text-slate-200">{selected.investable_thesis}</p>
                    </section>
                    <section className="min-w-0">
                      <h3 className="text-xs font-semibold uppercase text-slate-400">Portfolio transmission</h3>
                      <p className="mt-2 break-words text-sm leading-6 text-slate-700 dark:text-slate-200">{selected.portfolio_transmission}</p>
                    </section>
                    <section className="min-w-0">
                      <h3 className="text-xs font-semibold uppercase text-slate-400">Expected edge</h3>
                      <p className="mt-2 break-words text-sm leading-6 text-slate-700 dark:text-slate-200">{selected.expected_edge}</p>
                    </section>
                  </div>

                  <div className="grid gap-6 border-y border-slate-200 py-6 lg:grid-cols-3 dark:border-slate-800">
                  ...
                  <section>
                    <div className="flex flex-wrap items-center justify-between gap-3">
                      <h3 className="text-xs font-semibold uppercase text-slate-400">Evidence snapshot</h3>
                      <span className="text-xs text-slate-400">{selected.evidence_snapshot.length} items · {selected.evidence_refs.length} refs</span>
                    </div>
                  ...
                  <section className="grid gap-6 border-t border-slate-200 pt-6 lg:grid-cols-[minmax(0,1fr)_minmax(280px,0.7fr)] dark:border-slate-800">
                    <div>
                      <label htmlFor="review-reason" className="text-xs font-semibold uppercase text-slate-400">Review note</label>
                    ...
                    <div className="border-l-0 border-slate-200 lg:border-l lg:pl-6 dark:border-slate-800">
                      ...
                      {selected.shadow_experiment_id ? (
                        ...
                        <>
                          <div className="mt-3 grid grid-cols-2 gap-1 rounded-md border border-slate-200 p-1 dark:border-slate-700" role="group" aria-label="Shadow account basis">
                            {(["cash_only", "clone_portfolio"] as const).map((basis) => (
                              <button key={basis} type="button" onClick={() => setShadowBasis(basis)} className={`rounded px-2 py-1.5 text-xs font-medium ${shadowBasis === basis ? "bg-slate-900 text-white dark:bg-slate-100 dark:text-slate-950" : "text-slate-500"}`}>{basis === "cash_only" ? "Cash only" : "Clone portfolio"}</button>
                            ))}
                          </div>
                          ...
                          <button type="button" onClick={() => void startShadowTest()} disabled={busy === "shadow" || selected.status === "rejected" || selected.status === "expired" || (shadowBasis === "cash_only" && Number(startingCash) <= 0)} className="mt-2 inline-flex w-full items-center justify-center gap-2 rounded-md border border-slate-900 px-3 py-2 text-sm font-medium hover:bg-slate-900 hover:text-white disabled:cursor-not-allowed disabled:opacity-40 dark:border-slate-100 dark:hover:bg-slate-100 dark:hover:text-slate-950">
                            {busy === "shadow" ? <LoaderCircle className="h-4 w-4 animate-spin" /> : <FlaskConical className="h-4 w-4" />} Start shadow test
                          </button>
                          <p className="mt-2 text-xs leading-5 text-slate-500">Creates a simulated account only. It cannot place a real order or add a real holding.</p>
                        </>
                      )}
                    </div>
                  </section>
          ...
          <div className="grid gap-8 py-6 lg:grid-cols-[minmax(320px,0.45fr)_minmax(0,1fr)]">
            <div className="space-y-6">
              <section className="border border-slate-200 bg-white p-5 dark:border-slate-800 dark:bg-slate-950">
                    ...
                    <div key={source.source_type} className="py-3 first:pt-0 last:pb-0">
                      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                        ...
                        <button
                          ...
                          className="inline-flex h-9 shrink-0 items-center gap-2 rounded-md border border-slate-300 px-3 text-xs font-medium hover:border-slate-500 disabled:cursor-not-allowed disabled:opacity-40 dark:border-slate-700"
                        >
                      ...
                      <details className="mt-2 text-xs">
                        <summary className="cursor-pointer select-none text-slate-500 hover:text-slate-800 dark:hover:text-slate-200">
                          Review eligible securities
                        </summary>
                        <div className="mt-2 max-h-48 overflow-y-auto border-l border-slate-200 pl-3 dark:border-slate-800">
                            ...
                            .map((candidate) => (
                              <div key={`${source.source_type}-${candidate.security_id}`} className="flex items-baseline justify-between gap-3 py-1.5">
                                <p className="min-w-0 truncate">
                                  <span className="font-semibold text-slate-800 dark:text-slate-200">{candidate.ticker}</span>
                                  <span className="ml-2 text-slate-500">{candidate.entity_name}</span>
                                </p>
                          ...
                          {!source.eligible_count ? <p className="py-2 text-slate-500">No eligible stored securities.</p> : null}
                        </div>
                      </details>
                    </div>
                  ))}
                  {!importPreview && !importPreviewError ? <p className="py-3 text-xs text-slate-500">Loading import preview...</p> : null}
                ...
                <p className="mt-4 border-t border-slate-200 pt-3 text-xs leading-5 text-slate-500 dark:border-slate-800">
              ...
              <form onSubmit={addUniverseMember} className="border border-slate-200 bg-white p-5 dark:border-slate-800 dark:bg-slate-950">
                ...
                <div className="mt-4 grid gap-3">
                  ...
                  <label className="text-xs font-medium text-slate-500">Priority 0–1<input required type="number" min="0" max="1" step="0.05" value={priority} onChange={(event) => setPriority(event.target.value)} className={`${fieldClass} mt-1`} /></label>
                  <button disabled={busy === "add-member"} className="inline-flex items-center justify-center gap-2 rounded-md bg-slate-900 px-3 py-2 text-sm font-medium text-white disabled:opacity-50 dark:bg-slate-100 dark:text-slate-950">{busy === "add-member" ? <LoaderCircle className="h-4 w-4 animate-spin" /> : <Plus className="h-4 w-4" />} Add to universe</button>
                </div>
              </form>
              ...
              {settings ? (
                <section className="border border-slate-200 bg-white p-5 dark:border-slate-800 dark:bg-slate-950">
                  ...
                  <p className="mt-2 text-xs leading-5 text-slate-500">These limits apply to scheduled scans and are recorded on each run. Manual scans use the same per-run limits.</p>
                  <label className="mt-4 flex items-center justify-between gap-4 text-sm"><span>Scheduled discovery</span><input type="checkbox" checked={settings.opportunity_discovery.enabled} onChange={(event) => updateDiscoverySettings("enabled", event.target.checked)} className="h-4 w-4 accent-slate-900" /></label>
                  ...
                  <button type="button" onClick={() => void saveDiscoverySettings()} disabled={busy === "settings"} className="mt-4 inline-flex w-full items-center justify-center gap-2 rounded-md border border-slate-900 px-3 py-2 text-sm font-medium hover:bg-slate-900 hover:text-white disabled:opacity-50 dark:border-slate-100 dark:hover:bg-slate-100 dark:hover:text-slate-950">{busy === "settings" ? <LoaderCircle className="h-4 w-4 animate-spin" /> : <Check className="h-4 w-4" />} Apply controls</button>
            ...
            <section>
              <div className="flex flex-wrap items-end justify-between gap-4 border-b border-slate-200 pb-3 dark:border-slate-800">
                <div className="min-w-0"><h2 className="text-lg font-semibold">Investable universe</h2><p className="mt-1 break-words text-sm text-slate-500">This list is the explicit boundary of discovery coverage.</p></div>
                <span className="text-xs text-slate-400">{dueCount} due</span>
              </div>
              <div className="divide-y divide-slate-200 dark:divide-slate-800">
                {universe.map((member) => (
                  <div key={member.id} className="grid gap-4 py-4 sm:grid-cols-[minmax(0,1fr)_160px_auto] sm:items-center">
                    <div className="min-w-0">
                      <div className="flex items-center gap-2"><p className="font-semibold text-blue-700 dark:text-blue-300">{member.ticker}</p><span className="truncate text-sm text-slate-500">{member.entity_name}</span></div>
                      <p className="mt-1 text-xs text-slate-400">Last inspected: {formatDate(member.last_inspected_at)} · Next: {formatDate(member.next_inspection_at)}</p>
                      <p className="mt-1 truncate text-xs text-slate-500" title={member.origins.map((origin) => origin.label).join(", ")}>
                        Sources: {member.origins.length ? member.origins.map((origin) => origin.label).join(" · ") : displayValue(member.source)}
                      </p>
                    </div>
                    <label className="text-xs font-medium text-slate-500">Priority<input type="number" min="0" max="1" step="0.05" defaultValue={member.priority} onBlur={(event) => { const value = Number(event.target.value); if (value !== member.priority) void updateUniverseMember(member, { priority: value }); }} className={`${fieldClass} mt-1`} /></label>
                    <div className="flex items-center justify-end gap-2">
                      <button type="button" onClick={() => void updateUniverseMember(member, { enabled: !member.enabled })} disabled={busy === `member-${member.id}`} title={member.enabled ? "Pause discovery for this security" : "Enable discovery for this security"} className={`inline-flex h-9 items-center gap-2 rounded-md border px-3 text-sm font-medium ${member.enabled ? "border-emerald-300 text-emerald-700 dark:border-emerald-800 dark:text-emerald-300" : "border-slate-300 text-slate-500 dark:border-slate-700"}`}>{member.enabled ? <Check className="h-4 w-4" /> : <CircleDot className="h-4 w-4" />}{member.enabled ? "Enabled" : "Paused"}</button>
                      <button type="button" onClick={() => void removeUniverseMember(member)} disabled={busy === `member-${member.id}`} title={`Remove ${member.ticker} from discovery`} aria-label={`Remove ${member.ticker} from discovery`} className="inline-flex h-9 w-9 items-center justify-center rounded-md border border-slate-300 text-slate-500 hover:border-red-300 hover:text-red-600 dark:border-slate-700"><Trash2 className="h-4 w-4" /></button>
                    </div>
                  </div>
                ))}
                {!universe.length ? <p className="py-12 text-center text-sm text-slate-500">No securities are in the discovery universe.</p> : null}
              </div>
            </section>
          </div>
        ) : null}
        ...
        {!loading && tab === "runs" ? (
          <section className="py-6">
            <div className="border-b border-slate-200 pb-3 dark:border-slate-800">
              ...
              <p className="mt-1 text-sm text-slate-500">Point-in-time coverage, provider attempts, skips, failures, and the exact limits used.</p>
            </div>
            <div className="divide-y divide-slate-200 dark:divide-slate-800">
              {runs.map((run) => {
                ...
                return (
                  <article key={run.id} className="py-4">
                    <button type="button" onClick={() => setExpandedRun(expanded ? null : run.id)} className="grid w-full gap-3 text-left sm:grid-cols-[minmax(200px,1fr)_repeat(4,minmax(90px,auto))_auto] sm:items-center">
                      <div><div className="flex items-center gap-2"><span className={`h-2 w-2 rounded-full ${run.failed_count ? "bg-red-500" : run.status === "completed" ? "bg-emerald-500" : "bg-amber-500"}`} /><p className="text-sm font-semibold">{displayValue(run.status)}</p></div><p className="mt-1 text-xs text-slate-400">{formatDate(run.captured_at)}</p></div>
                    ...
                    {expanded ? (
                      <div className="mt-4 grid gap-5 border-l-2 border-slate-200 pl-4 lg:grid-cols-3 dark:border-slate-700">
                        ...
                        <p className="text-xs text-slate-500 lg:col-span-3">Detail: {displayValue(run.detail)} · started {formatDate(run.started_at)} · completed {formatDate(run.completed_at)}</p>
                      </div>
                    ) : null}
                  </article>
                );
              })}
              {!runs.length ? <p className="py-12 text-center text-sm text-slate-500">No discovery runs have been recorded.</p> : null}
            </div>
          </section>
        ) : null}
      </main>
...
function ListSection({ title, items, tone }: { title: string; items: string[]; tone: "neutral" | "warning" }) {
  return (
    <section>
      <h3 className={`text-xs font-semibold uppercase ${tone === "warning" ? "text-amber-600 dark:text-amber-400" : "text-slate-400"}`}>{title}</h3>
      {items.length ? <ul className="mt-3 space-y-2 text-sm text-slate-700 dark:text-slate-200">{items.map((item, index) => <li key={`${item}-${index}`} className="flex min-w-0 gap-2">{tone === "warning" ? <AlertTriangle className="mt-0.5 h-4 w-4 shrink-0 text-amber-500" /> : <CircleDot className="mt-1 h-3 w-3 shrink-0 text-slate-400" />}<span className="min-w-0 break-words">{item}</span></li>)}</ul> : <p className="mt-3 text-sm text-slate-400">None recorded.</p>}
    </section>
  );
}

function ActionButton({ icon: Icon, label, busy, onClick }: { icon: typeof Eye; label: string; busy: boolean; onClick: () => void }) {
  return <button type="button" onClick={onClick} disabled={busy} className="inline-flex items-center gap-2 rounded-md border border-slate-300 px-3 py-2 text-sm font-medium hover:bg-slate-100 disabled:opacity-50 dark:border-slate-700 dark:hover:bg-slate-900">{busy ? <LoaderCircle className="h-4 w-4 animate-spin" /> : <Icon className="h-4 w-4" />}{label}</button>;
}
...
function NumberSetting({ label, value, min, max, onChange }: { label: string; value: number; min: number; max: number; onChange: (value: number) => void }) {
  return <label className="text-xs font-medium text-slate-500">{label}<input type="number" min={min} max={max} value={value} onChange={(event) => onChange(Number(event.target.value))} className={`${fieldClass} mt-1`} /></label>;
}

function RunMetric({ label, value }: { label: string; value: string }) {
  return <div><p className="text-[10px] font-semibold uppercase text-slate-400">{label}</p><p className="mt-0.5 text-sm font-medium tabular-nums">{value}</p></div>;
}

function RunDetail({ title, items }: { title: string; items: Array<{ label: string; detail: string }> }) {
  return <section><h3 className="text-xs font-semibold uppercase text-slate-400">{title}</h3>{items.length ? <ul className="mt-2 space-y-2">{items.map((item, index) => <li key={`${item.label}-${index}`}><p className="text-xs font-medium">{item.label}</p><p className="mt-0.5 break-words text-xs leading-5 text-slate-500">{item.detail}</p></li>)}</ul> : <p className="mt-2 text-xs text-slate-400">None recorded.</p>}</section>;
}
...
function OutcomeSection({
  ...
  if (!observation) {
    return (
      <section className="border-b border-slate-200 pb-6 dark:border-slate-800">
        <h3 className="text-xs font-semibold uppercase text-slate-400">Point-in-time outcome</h3>
  ...
  return (
    <section className="border-b border-slate-200 pb-6 dark:border-slate-800">
      ...
      {evaluated ? (
        <div className="mt-4 grid grid-cols-2 gap-px bg-slate-200 sm:grid-cols-4 dark:bg-slate-800">
      ...
      {observations.length > 1 ? (
        <div className="mt-4 border-t border-slate-200 pt-3 dark:border-slate-800">
          <p className="text-[10px] font-semibold uppercase text-slate-400">Prior frozen observations</p>
          <div className="mt-2 divide-y divide-slate-200 dark:divide-slate-800">
            {observations.slice(1).map((item) => (
              <div key={item.id} className="grid gap-1 py-2 text-xs sm:grid-cols-[1fr_auto_auto] sm:items-center sm:gap-4">
                <span className="text-slate-500">Captured {formatDate(item.captured_at)}</span>
                <span>{displayValue(item.result_label ?? item.status)}</span>
                <span className="tabular-nums text-slate-500">{formatPct(item.excess_return_pct)} excess</span>
              </div>
            ))}
          </div>
        </div>
...
function OutcomeMetric({ label, value }: { label: string; value: string }) {
  return (
    <div className="bg-panel-muted px-3 py-3">
